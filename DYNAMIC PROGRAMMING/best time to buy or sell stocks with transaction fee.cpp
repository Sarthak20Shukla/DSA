//best time to buy or sell stocks with transaction fee
...
int f(int ind,int buy,vector<int>& prices,vector<vector<int>> &dp,int fee){
    ...
    int profit=0;
    if(buy){
        profit=max(prices[ind]-fee+f(ind+1,0,prices,dp,fee),0+f(ind+1,1,prices,dp,fee));
    } else {
        profit=max(-prices[ind]+f(ind+1,1,prices,dp,fee),0+f(ind+1,0,prices,dp,fee));
